...
export const Route=createFileRoute("/category/$slug")({head:({params})=>{const c=getCategory(params.slug);return pageHead({title:`${c?.name??"Category"} — Yogesh Jewellers`,description:c?.tagline??"Explore our jewellery collection.",path:`/category/${params.slug}`})},component:()=> <Category/>});
